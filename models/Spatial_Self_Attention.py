...
class SelfAttentionGroup(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock_SelfAttention(
                dim=dim,
...
class SpatialFeatureEnhancementModule(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.residual_group = SelfAttentionGroup(
            dim=dim,
            input_resolution=input_resolution,
            depth=depth,
            num_heads=num_heads,
    ...
    def forward(self, x, x_size):
        x_res = self.residual_group(x, x_size)
        x_img = self.patch_unembed(x_res, x_size)
        x_conv = self.conv(x_img)
        x_out = self.patch_embed(x_conv)
        return x_out + x
...
class Spatial_Self_Attention(nn.Module):
    def __init__(self,
        ...
        if drop_path is not None:
            dpr = list(drop_path)
            assert len(dpr) == sum(depths), "drop_path length must equal sum(depths)"
        else:
            dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]

        self.SFM = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = SpatialFeatureEnhancementModule(
                dim=embed_dim,
